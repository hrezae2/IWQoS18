...
uint32_t        Scale                     = 1;
double          load                      = 0.6;
uint32_t 	threshold 		  = 1e6;
uint32_t 	queueBytes 		  = 4e6;      // Number of bytes per queue port
...
void Setup_Workload(){
  	...
  	for (uint32_t i=0;i<AppIdxShort.size(); i=i+2){
		randomShort();
        	...
        	Map_Port[ServerNodeIdx].push_back(appPort);
      		...
      		SetupServerTraffic(nEnd[ServerNodeIdx],appPort,Curr_Start, 3);
		...
		NumShortFlows++;
                ...
                SetupServerTraffic(nEnd[ClientNodeIdx],appPort,Curr_Start, 3);
  ...
  for (uint32_t k=0;k < LongRequestsPerNode; k++){
        ...
        for (uint32_t i=0;i<AppIdxLong.size(); i=i+2){
                ...
                Prev_Start[ClientNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ServerNodeIdx],appPort,Curr_Start, 3);
                SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeLong,appPort,Curr_Start, 2);
		NumLongFlows++;
                Curr_Start = Prev_Start[ServerNodeIdx] + Seconds (DelayRandomlyLong->GetValue());
                Prev_Start[ServerNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ClientNodeIdx],appPort,Curr_Start, 3);
                SetupClientTraffic(nEnd[ServerNodeIdx],nEnd[ClientNodeIdx],FlowSizeLong,appPort,Curr_Start, 2);
		NumLongFlows++;
        }
  }
